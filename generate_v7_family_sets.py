...
def _sample_task(family, mode, rng):
    if mode == "benchmark":
        if family == "er":
            num_nodes = rng.randint(15, 18)
            p = round(rng.uniform(0.20, 0.30), 2)
            return family, (num_nodes, p), 55
        if family == "sp":
            num_repairable = rng.randint(13, 16)  # total nodes in [15, 18]
            return family, (num_repairable,), 55
        mesh_shape = rng.choice([(3, 5), (4, 4), (3, 6)])  # 15, 16, 18 nodes
...
def _build_tasks(family, n_instances, seed, h, mode):
    rng = random.Random(seed)
    tasks = []
    for i in range(n_instances):
        graph_type, params, iters = _sample_task(family, mode, rng)
...
def generate_one(out_path, family, n_instances, seed, h, workers, mode, split_name):
    print(f"\n=== {out_path} | family={family} | n={n_instances} | mode={mode} ===")
    started_at = datetime.now().isoformat()
    t0 = time.time()
    tasks = _build_tasks(family=family, n_instances=n_instances, seed=seed, h=h, mode=mode)
